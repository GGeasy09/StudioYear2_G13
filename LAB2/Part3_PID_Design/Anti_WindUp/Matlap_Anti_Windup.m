...
Setpoint = 180 + 360*10;
Initial_point = 0;
Method_name = ["Discerate Model with Aniti Windup","Continuous Model with Aniti Windup","Constant block with Aniti Windup","Discerate Model without Aniti Windup","Continuous Model without Aniti Windup","Constant block without Aniti Windup"];
simfile = 'Anti_Windup.slx'; 
load_system(simfile);

% Define the test matrix
Control_Methods = [2,5];  % e.g., PI and PID with Anti-Windup
Saturation_States = [1,2]; % 1 = No Saturation, 2 = Saturation Enabled

%% Pre-allocation
% We now have 4 total runs
...
ss_errors = zeros(total_runs, 1); % NEW: Steady State Error
...
for m = 1:length(Control_Methods)
    for s = 1:length(Saturation_States)
        ...
        % Assign variables to Workspace for Simulink to pick up
        Controller_Method = Control_Methods(m); 
        Saturation = Saturation_States(s); 
        ...
        if isempty(last_out), settling_times(run_idx) = 0;
        elseif last_out == length(time_signal), settling_times(run_idx) = NaN;
        else, settling_times(run_idx) = time_signal(last_out + 1);
        end
        
        % Create Label (UPDATED)
        sat_text = "No Sat"; 
        if Saturation == 2, sat_text = "Sat"; end
        
        method_str = Method_name(Controller_Method);
...
xlabel('Time (s)'); 
ylabel('Position (deg)');
title('System Response: Position vs. Time'); % <-- TITLE ADDED HERE
...
yline([12, -12], 'r--', 'Saturation');
grid on; 
xlabel('Time (s)'); 
ylabel('Voltage (V)');
title('Controller Effort vs. Time'); % <-- TITLE ADDED HERE
legend(legend_labels, 'Location', 'best');