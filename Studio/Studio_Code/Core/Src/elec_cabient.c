...
/* Read all sensor states — pull-up: pin reads 1 when sensor active */
void Gripper_ReadState(Gripper *g)
{
    g->is_full_open  = (uint8_t)HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_6);   /* PC6:  1=fully open  */
    g->is_full_close = (uint8_t)HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_12);  /* PB12: 1=fully closed actually gripper*/ 
    g->is_up         = (uint8_t)HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_2);   /* PC2:  1=up, 0=down -> full up when 0 */
}   
